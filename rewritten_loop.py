...
#fill value gives edge weight for created self loops, if string gives method e.g. mean of incoming edges
#does not build curvature values
def add_self_loops(
        edge_index: Tensor, edge_attr: OptTensor = None,
    ...
    if edge_attr is not None:
        if fill_value is None:
            loop_attr = edge_attr.new_full((N, ) + edge_attr.size()[1:], 1.) #returns vector of size [N,2], weight=1 
            ...
            loop_attr = loop_attr.repeat(*sizes) #shape: [N, ...]
...
#curvature defined for self_loops, either 1 or fill_value
def add_remaining_self_loops(
        edge_index: Tensor, edge_attr: OptTensor = None,curve: OptTensor = None,
        fill_value: Union[float, Tensor, str] = None,
        num_nodes: Optional[int] = None) -> Tuple[Tensor, OptTensor]:
    ...
    if edge_attr is not None:
        if fill_value is None:
            ...
            loop_curve = curve.new_full((N, ) + curve.size()[1:], 1.)

        elif isinstance(fill_value, (int, float)):
            loop_attr = edge_attr.new_full((N, ) + edge_attr.size()[1:],
                                           fill_value)
            loop_curve = curve.new_full((N, ) + curve.size()[1:], 1) #not fill_value??
        elif isinstance(fill_value, Tensor):
            loop_attr = fill_value.to(edge_attr.device, edge_attr.dtype)
            loop_curve = fill_value.to(curve.device, curve.dtype)
            
            if edge_attr.dim() != loop_attr.dim():
                loop_attr = loop_attr.unsqueeze(0)
            if curve.dim() != loop_curve.dim():
                loop_curve = loop_curve.unsqueeze(0)
                
            sizes = [N] + [1] * (loop_attr.dim() - 1)
            loop_attr = loop_attr.repeat(*sizes)
            loop_curve = loop_curve.repeat(*sizes)

        elif isinstance(fill_value, str):
            loop_attr = scatter(edge_attr, edge_index[1], dim=0, dim_size=N,
                                reduce=fill_value)
            loop_curve = scatter(curve, edge_index[1], dim=0, dim_size=N,
                                reduce=fill_value)
        else:
            raise AttributeError("No valid 'fill_value' provided")
        ...
        loop_curve[edge_index[0][inv_mask]] = curve[inv_mask]

        edge_attr = torch.cat([edge_attr[mask], loop_attr], dim=0)
        curve = torch.cat([curve[mask], loop_curve], dim=0)

    edge_index = torch.cat([edge_index[:, mask], loop_index], dim=1)
    return edge_index, edge_attr, curve